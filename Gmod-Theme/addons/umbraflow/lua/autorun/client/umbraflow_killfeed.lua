--[[---------------------------------------------------------------------------
	...
	What Fluent gets us that the stock feed doesn't: each notice is a pill on a
	dark acrylic surface with a 1px stroke, names use the scheme's text ramp
	...
	SourceScheme.res has. The stack right-aligns to the screen edge; entrances
	slide in from the right and swing slightly past before settling.

	The pill and that swing are the two places this file departs from the theme's
	tokens on purpose — see the shape block for why.
...
-- Shape and motion.
--
-- The cards are pills: the corner radius is half the card height rather than
-- the theme's --radius-card, so the ends are full semicircles. That is a
-- deliberate break from the stylesheets, which round to 7px — a pill is the one
-- shape here that isn't trying to look like a window, and the feed isn't chrome
-- the way the menu is. draw.RoundedBox clamps a radius to floor( h / 2 ) on its
-- own, so the height is simply handed to it and the clamp does the work.
local DUR		= 0.167	-- --dur
local FADE		= 0.5	-- how long the tail fade runs for
local MARGIN	= 16	-- gap to the screen edge the stack right-aligns against
local OVERSHOOT	= 1.7	-- how far past its resting place a card swings on entry
...
local function BuildFont()
	...
	surface.CreateFont( FONT, {
		...
		extended	= true,
		size		= math.max( 13, math.Round( ScrH() * 0.0165 ) ),
		weight		= 600,
		antialias	= true
	} )
...
-- it. VGUI has no rounded outline, and neither does draw. Both boxes are handed
-- a radius past what they can take, so each clamps to its own half-height and
-- the two come out concentric — an inset pill inside a pill.
local function StrokedPill( x, y, w, h, fill, stroke )

	draw.RoundedBox( h, x, y, w, h, stroke )
	draw.RoundedBox( h, x + 1, y + 1, w - 2, h - 2, fill )
...
local function DrawDeath( y, death, lifetime )
	...
	-- Entrance: a 40px slide in from the right, overshooting its resting place
	-- and settling back — a back-out curve rather than the flat ease-out of the
	-- stylesheets. Chrome shouldn't bounce, but a pill that slides to a dead
	-- stop looks stiff, and the small swing past is what sells it as light.
	local enter	= math.Clamp( age / DUR, 0, 1 )
	local back	= enter - 1
	local eased	= 1 + ( OVERSHOOT + 1 ) * back ^ 3 + OVERSHOOT * back ^ 2

	-- Fade on the eased value would inherit the overshoot and flicker past full
	-- opacity, so the alpha tracks the raw curve instead.
	local frac	= math.min( enter, math.Clamp( remaining / FADE, 0, 1 ) )
	...
	-- A pill needs more room at the ends than a rounded rect does: they curve
	-- away from the text, so padX stays well clear of padY — the extra width is
	-- what keeps the names off the curve.
	--
	-- Height has a floor these numbers can't get under: the kill icon sets it
	-- whenever it out-measures the text, and killicon draws at its own size with
	-- no scale of its own. Trimming padY past this point just pinches the names
	-- without shrinking the pill.
	local padX	= 14
	local padY	= 6
	local gap	= 10
	local slide	= ( 1 - eased ) * 40

	-- The pill packs left to right — name, icon, name — and right-aligns to the
	-- margin. The base feed instead centres the icon on an anchor and lets the
	-- names run out either side of it, which a pill can't do: its right edge has
	...
	local cardW	= padX * 2 + lead + iw + gap + rw
	local cardH	= math.max( ih, th ) + padY * 2
	local iconX	= padX + lead
	local radius = cardH / 2
	...
	surface.SetAlphaMultiplier( frac )
		...
		draw.RoundedBox( cardH, cardX - 1, cardY + 2, cardW + 2, cardH, SHADOW )
		StrokedPill( cardX, cardY, cardW, cardH, SURFACE, STROKE )
		...
		--
		-- This edge is the only accent on the pill. A tint was tried across the
		...
		--
		-- It insets to the radius, spanning the pill's flat underside and
		-- stopping where the ends curve away, rather than cutting a chord.
		if ( death.mine ) then
	...
	if ( death.left ) then
		draw.SimpleText( death.left, FONT, cardX + padX, midY, ColorAlpha( death.color1, alpha ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
	end

	draw.SimpleText( death.right, FONT, cardX + cardW - padX, midY, ColorAlpha( death.color2, alpha ), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
